...
| Phone docking (§4.11, `phoneAway` §5.4.1) | ✅ | ✅ | **Verified on hardware 2026-08-01** (branch `phone_dock_refactor`), anchor + watch + a host-BLE stand-in phone. Anchor: Dock Register 0x01/0x00, 2 s RSSI poll, notify-on-change, docked at −51 dBm, clean undock on unregister. Watch: `[PROX] Dock status: docked/UNDOCKED` read inside the query, `near_phone` fusion, 60 s `PHONE_AWAY_TOLERANCE_S` honoured then sustained enforcement (compliant for 11 polls docked → grace → `met=0` at 70 s). Anchor advertises throughout a held link (310–538 ads/link), so the watch can still reach it during a dock session. **One defect found and fixed:** `ble_gap_conn_rssi()` returns rc=0 with a *foreign* RSSI (−86…−109, the watch's) on the docking handle immediately after another link tears down, which produced spurious undocks — i.e. false enforcement against a compliant user. Now requires `DOCK_UNDOCK_CONFIRM_POLLS` (3 = 6 s, well inside the 60 s tolerance) consecutive bad samples before believing an undock. |
...
- **Toolchain: bump off the frozen ESP-IDF 4.4.7 platform (deferred; do eventually).** The build currently resolves to PlatformIO `platform = espressif32 @ 6.12.0` → Arduino-ESP32 **2.0.17** → **ESP-IDF 4.4.7**, which is the *newest of a dead line*: Espressif stopped maintaining the official PlatformIO `espressif32` platform, and the registry publishes no framework past `3.20017` (there is no 2.0.18 / IDF 4.4.8 package for PlatformIO at all). IDF 4.4.7 has a **WiFi-scan-under-BLE-coexistence heap leak on the ESP32-C3**: `esp_wifi_scan_start()` leaks ~1.5 KB/call, which ran the anchor to OOM `PANIC` every ~20 min (found via the NVS crash-ring; §diagnostics). It is **fixed in IDF 4.4.8** (changelog: *"Fixed scan get AP number issue"*, *"Fixed memory leak issue when BLE SCAN and other BLE events coexist"* [#13747], *"…BLE do not update SCAN status correctly for coexistence module on ESP32C3"*) and in the 5.x line. **Current mitigation (shipped):** the anchor's WiFi AP scan is bounded — 250 s interval, `ANCHOR_PROX_WIFI_MAX_SCANS = 8`, then the scan task self-terminates — turning the unbounded leak into a one-time ~12 KB cost (proximity impact is negligible: the anchor is stationary and its live WiFi RSSI is stale-gated out of Signal A between scans; the fingerprint learns WiFi APs from watch vectors). **Eventual fix:** migrate to the community **pioarduino** platform fork on **Arduino 3.x / IDF 5.x** (e.g. `platform = https://github.com/pioarduino/platform-espressif32/releases/download/53.03.13-1/platform-espressif32.zip` for IDF 5.3.2), which fixes the leak at the source; then `ANCHOR_PROX_WIFI_MAX_SCANS` can go back to `0` (unlimited). Not a version bump but a real migration — Arduino 3.x has breaking API changes (notably the LEDC/servo API: `ledcSetup`/`ledcAttachPin`/`ledcWrite(channel,…)` → `ledcAttach(pin,…)`, used by the anchor servo), and NimBLE-Arduino + the WiFi manager want re-validation against IDF 5.x. Do it on its own branch, off the lockstep batch.
  ...
  **The same upgrade would also improve guided calibration (§8.5).** On IDF 4.4.7's old NimBLE, the watch **cannot** do a central connect to the anchor while it holds the phone (peripheral) link — the two-role case crashes the host (`assert ble_hs_timer_exp` during service discovery). The shipped workaround (**Option A**) is that the watch queries the anchor only while the phone is **disconnected**: the app writes START, drops the watch link, the watch bursts autonomously, and the app reconnects afterward to read the result. That costs the live donut/score during the burst (it falls back to an elapsed-time ring, per §8.5's documented fallback). IDF 5.x's NimBLE handles concurrent peripheral+central far more robustly, so after the migration calibration could hold the phone link throughout and restore the **live** score/accepted-sample feedback — removing the disconnect-during-burst dance. Track this alongside the `MAX_SCANS=0` revert as things to reclaim post-upgrade.

  **Toolchain-bump spike (2026-07) — result: do NOT bump the watch; the only reason to upgrade would be to upgrade the anchor.** A throwaway spike (`spike/toolchain-bump`) upgraded the watch to pioarduino `55.03.311` (Arduino 3.3.11 / IDF 5.5.5, NimBLE-Arduino 2.5.0) and reproduced the three-way concurrency (peripheral + central + WiFi) to test the claim above. **The calibration crash is NOT fixed.** With the config adjusted so the central connect actually succeeds (`CONFIG_BT_CTRL_BLE_MAX_ACT` raised 3→5; at 3 the controller rejected every connect with HCI "Memory Capacity Exceeded", rc=519), the watch still panics in **`ble_hs_timer_exp` → `ble_gap_update_next_exp`** — the same NimBLE host-timer path as IDF 4.4.7 — ~4 s after the phone connects, every time. So the assumption above ("IDF 5.x's NimBLE handles concurrent peripheral+central far more robustly") does **not** hold for NimBLE-Arduino 2.5.0; **Option A stays.** The bump **does** fix the WiFi-scan heap leak (watch ran ~27 min with WiFi+BLE, heap flat at ~35 KB), but the watch's exposure to that leak is already small (scan caching) and the leak is primarily an anchor problem that already ships a mitigation. The bump also introduces new watch-side blockers on this hardware: a BOD boot brownout (had to disable BOD), heap exhaustion (had to cut `MAX_EVENTS_PER_DAY` 64→24), WiFi `ESP_ERR_WIFI_STATE` (Arduino 3.x rejects stacked/concurrent `WiFi.begin()`), and a BLE advertising start race. **Net: the watch stays on IDF 4.4.7 + Option A.** The only reason to take the pioarduino/IDF 5.x migration is for the **anchor** — to reclaim the heap-leak fix at the source and set `ANCHOR_PROX_WIFI_MAX_SCANS=0` — which is a separate migration with its own breaking changes (LEDC/servo). Full spike write-up, decoded backtrace, and config in `tests/version_bump.md`.
