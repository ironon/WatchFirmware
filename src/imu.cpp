...
#define MOTION_THRESHOLD_MG   48    // dynamic accel (gravity removed) to trigger; 16 mg/LSB at ±2g
                                    //   (low so normal-pace movement reliably forces an ENFORCEMENT re-check)
...
bool lis3dh_init() {
    ...
    // CTRL_REG3: route IA1 (general motion) to the INT1 pin. Motion is the only
    // interrupt generator — the former IA2 wrist-raise (look-to-wake) generator
    // has been removed.
    ...
    // CTRL_REG5: latch INT1 (LIR_INT1) so a motion wake survives until INT1_SRC
    // is read.
